...
def main(params: DictConfig):
    ...
    floors_ply_paths = sorted(
        glob.glob(os.path.join(params.graph_path, "floors", "*.ply"))
    )
    floors_info_paths = sorted(
        glob.glob(os.path.join(params.graph_path, "floors", "*.json"))
    )
    ...
    rooms_ply_paths = sorted(
        glob.glob(os.path.join(params.graph_path, "rooms", "*.ply"))
    )
    rooms_info_paths = sorted(
        glob.glob(os.path.join(params.graph_path, "rooms", "*.json"))
    )
    ...
    objects_ply_paths = sorted(
        glob.glob(os.path.join(params.graph_path, "objects", "*.ply"))
    )
    objects_info_paths = sorted(
        glob.glob(os.path.join(params.graph_path, "objects", "*.json"))
    )
    ...
    for ply_path, info_path in zip(objects_ply_paths, objects_info_paths):
        ...
        object_infos[object_info["object_id"]] = {
            ...
            if k
            in ["object_id", "name", "room_id", "object_height", "object_zero_level"]
        }
        ...
        cloud_xyz += floor_infos[room_infos[object_info["room_id"]]["floor_id"]][
            "viz_offset"
        ]
    ...
    for floor_id, floor_centroid_viz in floor_centroids_viz.items():
        p.add_mesh(
            pv.Sphere(center=tuple(floor_centroid_viz), radius=0.5), color="orange"
        )
    ...
    room_centroids_viz = {
        room_id: room_centroids[room_id] + [0.0, 3.5, 0]
        for room_id in room_infos.keys()
    }
    for room_id, room_centroid_viz in room_centroids_viz.items():
        p.add_mesh(
            pv.Sphere(center=tuple(room_centroid_viz), radius=0.25), color="blue"
        )
        p.add_mesh(
            pv.Line(
                tuple(floor_centroids_viz[room_infos[room_id]["floor_id"]]),
                tuple(room_centroid_viz),
            ),
            line_width=4,
        )
