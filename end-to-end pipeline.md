...
Object (hovsg/graph/object.py): object_id, room_id, name, gt_name (unused GT placeholder), vertices, embedding (single CLIP vector), pcd. save()/load() round-trip to .ply + .json.

Room: holds objects list, embeddings (k representative view-level CLIP vectors), represent_images, plus room-type inference and per-class containment-belief methods.
...
Location	Change
hovsg/graph/object.py	Persist object labels, all five p_/u_ signal pairs, the combined p_sem_bar/u_sem_bar pair, and the raw detection/cross-view evidence.
...
Extend Object.__init__/save/load with the object-level signal fields and their raw evidence.
Use the pure helpers in hovsg/utils/uncertainty.py and cross_view_consistency.py to calculate the paper-defined signals with explicit undefined-input handling.
identify_object() returns the similarity vector alongside the label, avoiding a second CLIP/text lookup. segment_objects() stores detection, semantic, membership, and cross-view values on the Object; the whole-graph pass adds coherence and the combined semantic estimator.
Use Graph.recompute_semantic_uncertainty() once in build_graph(), iterating self.objects to refresh scores from the stored embedding.
